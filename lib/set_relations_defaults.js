...
function setRelationsDefaults(model) {
  ...
  model.constructor.relations.forEach(relation => {
    ...
    if (!relation.foreignKey) {
      if (relation.isHasMany) {
        relation.foreignKey = (0, _inflection.foreignKey)((0, _inflection.singularize)(relation.propertyName)) + 's';
      } else if (relation.isHasOne) {
        relation.foreignKey = (0, _inflection.foreignKey)(relation.propertyName);
      }
    }
